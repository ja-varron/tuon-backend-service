...
async def create_profile(db: AsyncSession, user_id: str, email: str, first_name: str, middle_name: str, last_name: str, role: str, institution_id: str, examinee_id_number: str | None):
    """
    ...
    sql_statement = text(f"""
        INSERT INTO {PROFILES_TABLE} (user_id, email, first_name, middle_name, last_name, role, institution_id, examinee_id_number)
        VALUES (:user_id, :email, :first_name, :middle_name, :last_name, :role, :institution_id, :examinee_id_number)
    """)
...
async def examinee_id_exists(db: AsyncSession, institution_id: str, examinee_id_number: str):
    sql_statement = text(f"""
        SELECT EXISTS(
            SELECT 1
            FROM {PROFILES_TABLE}
            WHERE institution_id = :institution_id
            AND examinee_id_number = :examinee_id_number
            AND role = 'student'
        )
    """)
